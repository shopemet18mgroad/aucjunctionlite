...
class BuyerAuction_furniture extends CI_Controller {
	...
	if(!$this->session->has_userdata('username')|| $this->session->userdata('auth') != "BUYER"){
		...
		if($this->uri->segment(3) == "TA"){
			...
			$config["total_rows"] = $this->Admin_model->get_count_cat('auction','Furniture');
		}
		...
		if($this->uri->segment(3) == "TA"){
			$data['sqldata1'] = $this->Admin_model->get_auctionlist_todayfurniture($config["per_page"], $page, $date);
		}else{
			$data['sqldata1'] = $this->Admin_model->get_auctionlistfurniture($config["per_page"], $page);
		}
