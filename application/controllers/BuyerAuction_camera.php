...
class BuyerAuction_camera extends CI_Controller {
	...
	if(!$this->session->has_userdata('username')|| $this->session->userdata('auth') != "BUYER"){
		...
		$config = array();
        $config["base_url"] = base_url() . "BuyerAuction_camera/index/".$this->uri->segment(3)."/";
		if($this->uri->segment(3) == "TA"){
			$config["total_rows"] = count($this->Admin_model->get_auction_today($date));
		}else{
			$config["total_rows"] = $this->Admin_model->get_count_cat('auction','Camera');
		}
		...
		if($this->uri->segment(3) == "TA"){
			$data['sqldata1'] = $this->Admin_model->get_auctionlist_todaycamera($config["per_page"], $page, $date);
		}else{
			$data['sqldata1'] = $this->Admin_model->get_auctionlistcamera($config["per_page"], $page);
		}
	...
	//print_r($data['sqldata1']);die;
		...
		$this->load->view('buyer/auctionlist_camera',$data);
