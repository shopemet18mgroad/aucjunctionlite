...
class BuyerAuction_properties extends CI_Controller {
	...
	if(!$this->session->has_userdata('username')|| $this->session->userdata('auth') != "BUYER"){
		...
		if($this->uri->segment(3) == "TA"){
			$data['sqldata1'] = $this->Admin_model->get_auctionlist_todayproperties($config["per_page"], $page, $date);
		}else{
			$data['sqldata1'] = $this->Admin_model->get_auctionlistproperties($config["per_page"], $page);
		}
