...
class BuyerAuction_search extends CI_Controller {
	...
	if(!$this->session->has_userdata('username')|| $this->session->userdata('auth') != "BUYER"){
		...
		$config = array();
        $config["base_url"] = base_url() . "BuyerAuction_search/index/".$this->uri->segment(3)."/";
		if($this->uri->segment(3) == "TA"){
			$config["total_rows"] = count($this->Admin_model->get_auction_today($date));
		}else{
			$config["total_rows"] = $this->Admin_model->get_count_product('addlot',$iproductname);
		}
       //print_r($config["total_rows"]); die;
		...
		if($this->uri->segment(3) == "TA"){
			$data['sqldata1'] = $this->Admin_model->get_auctionlist_todaysearch($config["per_page"], $page, $date,$iproductname);
		}else{
			$data['sqldata1'] = $this->Admin_model->get_auctionlistsearch($config["per_page"],$page,$iproductname);
		}

	

		$this->load->view('buyer/header',$sess);
		$this->load->view('buyer/auctionlist_search',$data);
